...
export class CreateStudentGuardianComponent extends subscribedContainerMixin(formMixin()) {
  ...
  get guardians() {
    return this.itemForm.get('guardians') as FormArray;
  }
  ...
  buildGuardianProfile(): FormGroup {
    return this.fb.group({
      ...
      email: ['', [
        Validators.email,
        Validators.required,
        Validators.pattern(CustomValidators.emailRegExp),
  ...
  submitGuardianForm() {
    ...
    if(this.itemForm.valid) {
      this.studentId$.pipe(
        concatMap(id =>
          forkJoin(this.guardians.controls.map(guardian => this.studentGuardian.submit({
            ...guardian,
            ['student_id']: id
          }))))
      ).subscribe({
